...
/**
 * Created by Gadour on 11/05/2017.
 */
...
public class BluetoothDataService extends Service {
    ...
    private String customInput = "test";
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && intent.hasExtra("btData")) {
            customInput = intent.getStringExtra("btData");
        }
...
//                }
                ...
                if (msg.what == handlerState) {                                     //if message is what we want
                    ...
                    System.out.println("GADOUUUUUUUR");
                    Log.d("RECORDED", recDataString.toString());
                    isMade(recDataString.toString());
        ...
        if(mConnectingThread == null){
            ...
            if(!customInput.equals("test")) {
                mConnectedThread.write(customInput);
    ...
    private void checkBTState() {
        ...
        if (btAdapter == null) {
            ...
            if (btAdapter.isEnabled()) {
                ...
                try {
                    BluetoothDevice device = btAdapter.getRemoteDevice(User.getInstance().getBoitier().getMacAddress());
                    Log.d("DEBUG BT", "ATTEMPTING TO CONNECT TO REMOTE DEVICE : " + User.getInstance().getBoitier().getMacAddress());

                    mConnectingThread = new ConnectingThread(device);
                    mConnectingThread.start();

                    if(mConnectingThread.isAlive()){
                        isRuning("running");
                    }
    ...
    private class ConnectingThread extends Thread {
        ...
        public ConnectingThread(BluetoothDevice device) {
            ...
            Log.d("DEBUG BT", "MAC ADDRESS : " + User.getInstance().getBoitier().getMacAddress());
...
//                    bluetoothIn.obtainMessage(handlerState, bytes, -1, buffer).sendToTarget();     // Send to message queue Handler
//                } catch (IOException e) {
//                    Log.d("DEBUG BT", e.toString());
//                    Log.d("BT SERVICE", "UNABLE TO READ/WRITE, STOPPING SERVICE");
//                    stopSelf();
...
//        }
    ...
    private static void isRuning(String msg) {
        Intent intent = new Intent("isServiceRunning");
        intent.putExtra("Status", msg);
        LocalBroadcastManager.getInstance(context).sendBroadcast(intent);
    }

    private static void isMade(String msg){
        Intent intent = new Intent("isRecipeMade");
